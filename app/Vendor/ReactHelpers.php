...
{
    $content = file_get_contents(dirname(__DIR__) . '/webroot/react/dist/.vite/manifest.json');
...
function assetUrl(string $entry): string
{
    ...
    return isset($manifest[$entry])
        ? Router::url('/', true).'react/dist/' . $manifest[$entry]['file']
        : '';
}
...
function importsUrls(string $entry): array
{
    ...
    if (!empty($manifest[$entry]['imports'])) {
        foreach ($manifest[$entry]['imports'] as $imports) {
            $urls[] = Router::url('/', true).'react/dist/' . $manifest[$imports]['file'];
        }
    }
    return $urls;
}
...
function cssUrls(string $entry): array
{
    ...
    if (!empty($manifest[$entry]['css'])) {
        foreach ($manifest[$entry]['css'] as $file) {
            $urls[] = Router::url('/', true).'react/dist/' . $file;
        }
    }
    return $urls;
}
